...
**Arch:** $arch
**Status: $status_badge**
$new_product_issues$existing_product_issues$existing_soft_product_issues$new_openqa_issues$existing_openqa_issues$todo_issues""")
...
def issue_state(result_list, include_softfails):
    # if any result was still failing the issue is regarded as existing
    if [i for i in result_list if i['state'] == 'STILL_FAILING']:
        return 'existing'
    elif include_softfails and [i for i in result_list if i['state'] == 'STILL_SOFT_FAILING']:
        return 'existing_soft'
    else:
        return 'new'
...
class ArchReport(object):
    ...
    def __init__(self, arch, results, args, root_url, progress_browser, bugzilla_browser, test_browser):
        ...
        for bugref, result_list in iteritems(results_by_bugref):
            ...
            if (bug['state'] in soft_fail_states and args.include_softfails) or bug['state'] not in soft_fail_states:
                ...
                self.issues[issue_state(result_list, self.args.include_softfails)][issue_type(bugref)]\
    ...
    def __str__(self):
        ...
        return openqa_review_report_arch_template.substitute({
            ...
            'existing_soft_product_issues':
                issue_listing('***Existing soft-failed Product bugs:***', self.issues['existing_soft']['product'], self.args.show_empty),
