...
change_state = {
    ...
    (
        "result_failed",
        "result_passed",
    ): "FIXED",  # fixed, maybe spurious, false positive
    ("result_softfailed", "result_passed"): "FIXED",
    (
        "result_failed",
        "result_failed",
    ): "STILL_FAILING",  # still failing or partial improve, partial degrade
    ("result_softfailed", "result_softfailed"): "STILL_SOFT_FAILING",
    ("result_failed", "result_softfailed"): "IMPROVED",
    (
        "result_passed",
        "result_passed",
    ): "STABLE",  # ignore or crosscheck if not fals positive
}
...
def get_test_details(entry):
    ...
    return {
        ...
        "failedmodules": [
            {
                "href": find_module_href(m),
                "name": m.text.strip(),
                "needles": get_failed_needles(m),
            }
...
def get_test_bugref(entry):
    ...
    return {
        "bugref": re.search(r"\S+#([0-9]+)", bugref.i["title"]).group(),
    ...
    if output_state_results:
        ...
        for state in interesting_states_names:
            print(
                ...
                % (
                    state,
                    ", ".join(k for k, v in iteritems(interesting_states) if v["state"] == state),
                )
            )
    ...
    previous_results_list = [
        (
            i.td["id"],
            {
                ...
                "build": i.find(class_="build").text,
            },
        )
...
""" % (
    ...
    product_entries = OrderedDict(
        [
            ...
            (
                "short_desc",
                "[Build %s] openQA test fails%s" % (build, " in %s" % module if module else ""),
            ),
    ...
    test_entries = OrderedDict(
        [
            (
                "issue[subject]",
                "[Build %s] test %sfails" % (build, module + " " if module else ""),
            ),
...
class Issue(object):
    ...
    def __init__(
        ...
        bugref,
        bugref_href,
        query_issue_status=False,
        progress_browser=None,
        bugzilla_browser=None,
    ):
        ...
        for k, v in iteritems(results):
            if v["state"] in soft_fail_states:
                try:
                    ...
                    assert module_name, "could not find a module name within %s in job %s" % (
                        module_url,
                        v["href"],
                    )
        ...
        return openqa_review_report_arch_template.substitute(
            {
                ...
                "new_product_issues": issue_listing(
                    "**New Product bugs:**",
                    self.issues["new"]["product"],
                    self.args.show_empty,
                ),
                "existing_product_issues": issue_listing(
                    "**Existing Product bugs:**",
                    self.issues["existing"]["product"],
                    self.args.show_empty,
                ),
...
class Report(object):
    ...
    def _next_label(self):
        return "%s %i%%" % (
            self._label,
            self._progress * 100 / len(self.job_groups.keys()),
        )
