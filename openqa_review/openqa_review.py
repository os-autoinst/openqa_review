...
def issue_listing(header, issues, show_empty=True, no_headers=False):
    ...
    if not show_empty and len(issues) == 0:
        return ""
    if no_headers:
        issue_list_item = lambda issue : '* ' + header + ' ' + issue.stringify() + "\n"
        return "".join(map(issue_list_item, issues))
    return "\n" + header + "\n\n" + "".join(map(str, issues)) + "\n"
...
class ArchReport(object):
    ...
    def has_todo_issues(self):
        return True if (self.issues["new"]["todo"] or self.issues["existing"]["todo"]) else False
    ...
    def _todo_issues_str(self):
        ...
        if self.args.todo_only:
            todo_issues = issue_listing("**new**", self.issues["new"]["todo"], self.args.show_empty, no_headers=True)
            todo_issues += issue_listing("**existing**", self.issues["existing"]["todo"], self.args.show_empty, no_headers=True)
            return todo_issues
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=False,
        dest="todo_only",
        help="List inly TODO items as one liners",
    )
    parser.add_argument(
        "--include-softfails",
        action="store_true",
    ...
    def __str__(self):
        ...
        for k, v in self.report.items():
            if not self.args.skip_passed or type(v) is not ProductReport or not v.is_passed():
                generated_report = str(v)
                if len(generated_report):
                    if self.args.todo_only:
                        report_str += "* %s:\n%s" % (k, generated_report)
                    else:
                        report_str += "# %s\n\n%s\n---\n" % (k, generated_report)
        return report_str
