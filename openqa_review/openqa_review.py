...
openqa_issue_comment = Template(
    """
This bug is still referenced in a failing openQA test: [$name]($url).

Please check that the test failure was addressed, or move the openQA job group to "Released" or "EOL" (End-of-Life).

Alternatively delete [obsolete bugref comments]($jobsearch) or add new comments, e.g. `label:wontfix:$bugref`. Test code using `record_soft_failure` needs to be removed or use '$no_reminder_pattern'.

Note: If `auto_review:` is or was used, it may be too generic and bugrefs added by it are not automatically removed.
...
def reminder_comment_on_issue(ie: IssueEntry, args: argparse.Namespace) -> None:
    ...
    if (datetime.datetime.now(tz=datetime.UTC) - last_comment_date).days >= threshold:
        ...
        pattern = {
            ...
            "bugref": issue.bugref,
            "jobsearch": "https://openqa.opensuse.org/tests?comment=" + bugref.replace('#', '%23'),
        }
