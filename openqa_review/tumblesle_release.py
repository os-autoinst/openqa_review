...
class TumblesleRelease(object):
        ...
        for t in range(tries):
            try:
                self.notify_channel.basic_publish(
                    exchange="pubsub",
                    routing_key=".".join([self.notify_topic, topic]),
                    body=body,
                )
                break
    ...
    def retrieve_jobs_by_result(self, build):
        ...
        jobs_build = self.browser.get_json(
            "/api/v1/jobs?state=done&latest=1&build=%s&group_id=%s" % (build, group_id),
            cache=self.args.load,
        )["jobs"]
    ...
    def check_last_builds(self):
        ...
        if passed["last"] >= passed["released"] and hard_failed["last"] <= hard_failed["released"]:
            ...
            self.notify(
                {"build": build["last"], "new_failures": list(new_failures)},
                topic="regression",
            )
    ...
    def update_symlinks(self, build_dest):
        ...
        for i in glob.glob(build_dest + "*/*"):
            tgt = os.path.join(
                os.path.dirname(i),
                os.path.basename(i).replace(self.release_build, "CURRENT"),
            )
